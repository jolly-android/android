...
class RepoPagingDataSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, RepoModel> {
        ...
        return try {
            val response = repoService.getItems(page=page.toString(), type = query, params.loadSize.toString())
            val result = RepoResponse(items = response)
            LoadResult.Page(
                data = result.items,
                prevKey = if (page == STARTING_PAGE_INDEX) null else page.minus(1),
                nextKey = if (result.items.isEmpty()) null else page.plus(1)
            )
